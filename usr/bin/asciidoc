...
def parse_entry(entry, dict=None, unquote=False, unique_values=False,
    ...
    if mo:  # name=value entry.
        if mo.group(1):
            ...
            if escape_delimiter:
                name = name.replace(r'\=', '=')  # Un-escape \= in name.
...
def system(name, args, is_macro=False, attrs=None):
    ...
    if name not in ('eval', 'eval3', 'sys', 'sys2', 'sys3', 'include',
                    'include1', 'counter', 'counter2', 'set', 'set2', 'template'):
        if is_macro:
...
class AttributeEntry:
    ...
    def translate():
        ...
        if attr.name2 is not None:
            ...
            # Normal attribute.
            if attr.name[-1] == '!':
                # Names like name! un-define the attribute.
            ...
            if attr.value is not None:
                ...
                if mo:
                    # Inline pass-through syntax.
                    attr.subs = mo.group('attrs')
                    attr.value = mo.group('value')  # Pass-through.
                else:
    ...
    def parse(lines):
        ...
        if not result:
            ...
            if ul[:2] not in Title.underlines:
                return False
            # Length of underline must be within +/- 3 of title. Next, test for backward compatibility.
            if not ((ul_len-3 < title_len < ul_len+3) or (ul_len-3 < len(title) < ul_len+3)):
                return False
...
class Table(AbstractBlock):
    ...
    def parse_psv_dsv(self, text):
        ...
        data += text[start:]
        append_cell(data, span, op, align, style)
        # We expect a dummy blank item preceding the first PSV cell.
        if format == 'psv':
    ...
    def translate(self):
        ...
        # substituted already inline passthroughs) unique placeholders are used
        ...
        def subs_func(mo):
            ...
            if 'attrlist' in d:
                if d['attrlist'] in (None, ''):
                    ...
                    if self.prefix == '':
                        # Un-escape ] characters in inline macros.
            ...
            if name == 'image' and '1' in d:
                d['alt'] = d['1']
            # Un-escape special characters in LaTeX target file names.
