...
def SAXDefaultVersion(version):
    """DEPRECATED: Use parser option XML_PARSE_SAX1.  Set the
...
def namePop(ctxt):
    """DEPRECATED: Internal function, do not use.  Pops the top
       element name from the name stack """
...
def namePush(ctxt, value):
    """DEPRECATED: Internal function, do not use.  Pushes a new
       element name on top of the name stack """
    if ctxt is None: ctxt__o = None
    else: ctxt__o = ctxt._o
...
def nodePop(ctxt):
    """DEPRECATED: Internal function, do not use.  Pops the top
       element node from the node stack """
    if ctxt is None: ctxt__o = None
    else: ctxt__o = ctxt._o
...
def nodePush(ctxt, value):
    """DEPRECATED: Internal function, do not use.  Pushes a new
       element node on top of the node stack """
    if ctxt is None: ctxt__o = None
    else: ctxt__o = ctxt._o
...
def URIEscapeStr(str, list):
    """This routine escapes a string to hex, ignoring reserved
      characters (a-z, A-Z, 0-9, "@-_.!~*'()") and the characters
...
class parserCtxt(parserCtxtCore):
    ...
    def htmlParseCharRef(self):
        """DEPRECATED: Internal function, don't use.  parse Reference
    ...
    def parseCharData(self, cdata):
        """DEPRECATED: Internal function, don't use.  Parse character
          data. Always makes progress if the first char isn't '<' or
          '&'.  if we are within a CDATA section ']]>' marks an end
          of section.  The right angle bracket (>) may be represented
          using the string "&gt;", and must, for compatibility, be
          escaped using "&gt;" or a character reference when it
          appears in the string "]]>" in content, when that string is
          not marking the end of a CDATA section.  [14] CharData ::=
           [^<&]* - ([^<&]* ']]>' [^<&]*) """
    ...
    def parseEntityRef(self):
        """DEPRECATED: Internal function, don't use.  Parse an entitiy
          reference. Always consumes '&'.  [68] EntityRef ::= '&'
          Name ';'  [ WFC: Entity Declared ] In a document without
          any DTD, a document with only an internal DTD subset which
          contains no parameter entity references, or a document with
          "standalone='yes'", the Name given in the entity reference
          must match that in an entity declaration, except that
          well-formed documents need not declare any of the following
          entities: amp, lt, gt, apos, quot.  The declaration of a
          parameter entity must precede any reference to it. 
          Similarly, the declaration of a general entity must precede
          any reference to it which appears in a default value in an
          attribute-list declaration. Note that if entities are
          declared in the external subset or in external parameter
          entities, a non-validating processor is not obligated to
          read and process their declarations; for such documents,
    ...
    def skipBlankChars(self):
        """DEPRECATED: Internal function, do not use.  skip all blanks
          character found at that point in the input streams. It pops
          up finished entities in the process if allowable at that
           point. """
        ret = libxml2mod.xmlSkipBlankChars(self._o)
        return ret

    def stringDecodeEntities(self, str, what, end, end2, end3):
        """DEPRECATED: Internal function, don't use.  Takes a entity
          string content and process to do the adequate
          substitutions.  [67] Reference ::= EntityRef | CharRef 
           [69] PEReference ::= '%' Name ';' """
